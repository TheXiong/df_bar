        ...
        <div class='bg-white h-100 mx-1 p-1 shadowed'>
          ...
          <el-tabs type='border-card'>
            <el-tab-pane label="用户管理">
              ...
              <template>
                <el-table ref="multipleTable" :data="tableData" tooltip-effect="dark" highlight-current-row style="width: 100%;"
                  ...
                  <el-table-column fixed='right' label='操作' width='300'>
                    <template slot-scope='scope'>
                      <el-button icon="el-icon-edit" @click='handleSetRoles(scope.$index, scope.row)' type='text' size='small'>分配角色</el-button>
                      <el-button icon="el-icon-edit" @click='handleViewRoles(scope.$index, scope.row)' type='text' size='small'>查看角色</el-button>
              ...
              <el-dialog title='设置密码' :visible.sync='dialogPwdVisible' width='50%' :before-close='handleClose'>
              ...
              <el-dialog title='分配角色' :visible.sync='dialogRoleVisible' width='50%' :before-close='handleClose'>
                ...
                <el-row>
                  <el-col :span='24' class='toolbar' style='padding-bottom: 0px;'>
                    <div class='pagination'>
                      <el-pagination @current-change='handleCurrentChangeRole' layout='prev, pager, next' :total='totalRole'
                        :page-size='20' :current-page='searchFormRole.page'>
                      </el-pagination>
                    </div>
                  </el-col>
                </el-row>
              ...
              <el-dialog title='查看角色' :visible.sync='dialogRoleUserVisible' width='50%' :before-close='handleClose'>
                <el-table ref="multipleTable" :data="tableRoleUserData" tooltip-effect="dark" highlight-current-row
                  style="width: 100%;" @selection-change="handleSelectionChangeRoleUser">
                  <el-table-column type='selection' width='55'>
                  </el-table-column>
                  <el-table-column type='index' width='60'>
                  </el-table-column>
                  ...
                  <el-col :span='24' class='toolbar' style='padding-bottom: 0px;'>
                    <div class='pagination'>
                      <el-pagination @current-change='handleCurrentChangeRoleUser' layout='prev, pager, next' :total='totalRoleUser'
                        :page-size='20' :current-page='searchFormRoleUser.page'>
                      </el-pagination>
                    </div>
                  </el-col>
                </el-row>
    ...
    methods: {
      ...
      handleRoleSearch: function() {
        ...
        if (self.searchFormRole.page == undefined) {
          self.searchFormRole.page = 1;
        }
        self.$axios.post('/BaseRole/ListByPage?page=' + self.searchFormRole.page + "&limit=" + self.searchFormRole.limits,
          self.searchFormRole).then(function(response) {
          self.tableRoleData = response.data.data;
          self.totalRole = response.data.count;
      ...
      handleUserRoleSearch: function() {
        ...
        if (self.searchFormRoleUser.page == undefined) {
          self.searchFormRoleUser.page = 1;
        }
        self.$axios.post('/BaseRole/ListByUserId/'+this.userId+'?page=' + self.searchFormRoleUser.page + "&limit=" + self.searchFormRoleUser.limits,
      ...
      handleSearch: function() {
        ...
        self.$axios.post('/BaseBuser/ListByPage?page=' + self.searchForm.page + "&limit=" + self.searchForm.limits,
            self.searchForm)
          .then(function(response) {
      ...
      handleSetRoleSave: function() {
        ...
        if (self.multipleSelectionRole.length == 0) {
          ...
          this.$axios.post('/BaseBuser/SetRoles/' + this.userId, qs.stringify({
            'ids': ids
          }, {
            indices: false
          })).then((res) => {
      ...
      handleSave: function(formName) {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            this.$axios.post('/BaseBuser/Create', this.addForm).then((res) => {
              if (res.data.code == 200) {
                self.$message.success(res.data.msg);
                self.handleSearch();
      ...
      handleUpdatePwd: function(formName) {
        ...
        self.$refs[formName].validate((valid) => {
          if (valid) {
            self.$axios.post('/BaseBuser/SetPwd', this.pwdForm).then((res) => {
              if (res.data.code == 200) {
                self.$message.success(res.data.msg);
      ...
      handleUpdate: function(formName) {
        let self = this;
        this.$axios.post('/BaseBuser/Edit', this.editForm).then((res) => {
          self.$message.success(res.data.msg);
          self.handleSearch();
      ...
      handleDel: function() {
        ...
        this.$confirm('确认删除该记录吗?', '提示', {
          ...
          self.$axios.post('/BaseBuser/SetDeleted', qs.stringify({
            'ids': ids
          }, {
            indices: false
          })).then(function(response) {
      ...
      handleSingleDelete(index, row) {
        ...
        self.$confirm('确认删除该记录吗?', '提示', {
          ...
          self.$axios.post('/BaseBuser/SetDeleted', qs.stringify(para, {
              indices: false
            }))
            .then(function(response) {
              self.$message.success(response.data.msg);
