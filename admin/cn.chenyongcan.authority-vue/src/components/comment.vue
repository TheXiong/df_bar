  ...
  export default {
    ...
    methods: {
      ...
      handleSearch: function() {
        ...
        self.$axios.post('/BaseComment/ListByPage?page=' + self.searchForm.page + "&limit=" + self.searchForm.limits, self.searchForm).then(function(response) {
      ...
      handleSave: function(formName) {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            this.$axios.post('/BaseComment/Create', this.addForm).then((res) => {
      ...
      handleAuditStatus: function(index, row) {
        let self = this;
        this.$axios.post('/BaseComment/Audit/'+row.id).then((res) => {
          if (res.data.code == 200) {
            self.$message.success(res.data.msg);
            self.handleSearch();
          } else {
      ...
      handleUpdate: function(formName) {
        let self = this;
        this.$axios.post('/BaseComment/Edit', this.editForm).then((res) => {
          self.$message.success(res.data.msg);
          self.handleSearch();
      ...
      handleDel: function() {
        ...
        this.$confirm('确认删除该记录吗?', '提示', {
          ...
          self.$axios.post('/BaseComment/SetDeleted', qs.stringify({
            'ids': ids
          }, {
            indices: false
      ...
      handleSingleDelete(index, row) {
        ...
        self.$confirm('确认删除该记录吗?', '提示', {
          ...
          axios.post('/BaseComment/SetDeleted', qs.stringify(para))
            .then(function(response) {
              self.$message.success(response.data.msg);
              self.handleSearch();
            })
