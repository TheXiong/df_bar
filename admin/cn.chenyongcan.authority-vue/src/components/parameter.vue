            ...
            <el-tab-pane label="参数设置">
              ...
              <template>
                <el-table ref="multipleTable" :data="tableData" tooltip-effect="dark" highlight-current-row style="width: 100%;"
                  ...
                  <el-table-column prop='content' label='值' width='180'></el-table-column>
  ...
  export default {
    ...
    methods: {
      ...
      handleSearch: function() {
        ...
        self.$axios.post('/BaseParameter/ListByPage?page=' + self.searchForm.page + "&limit=" + self.searchForm.limits, self.searchForm).then(function(response) {
      ...
      handleSave: function(formName) {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            this.$axios.post('/BaseParameter/Create', this.addForm).then((res) => {
      ...
      handleUpdate: function(formName) {
        let self = this;
        this.$axios.post('/BaseParameter/Edit', this.editForm).then((res) => {
          self.$message.success(res.data.msg);
          self.handleSearch();
      ...
      handleDel: function() {
        ...
        this.$confirm('确认删除该记录吗?', '提示', {
          ...
          self.$axios.post('/BaseParameter/SetDeleted', qs.stringify({
            'ids': ids
          }, {
            indices: false
      ...
      handleSingleDelete(index, row) {
        ...
        self.$confirm('确认删除该记录吗?', '提示', {
          ...
          axios.post('/BaseParameter/SetDeleted', qs.stringify(para))
            .then(function(response) {
              self.$message.success(response.data.msg);
              self.handleSearch();
            })
      ...
      handleExport: function() {
        axios.post('/BaseParameter/export')
          .then(function(response) {
            self.$message.success(res.data.msg);
            self.handleSearch();
          })
