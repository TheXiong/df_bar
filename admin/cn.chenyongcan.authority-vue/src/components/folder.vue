  ...
  export default {
    ...
    methods: {
      ...
      handleSearch: function() {
        ...
        self.$axios.post('/BaseFolder/ListByPage?page=' + self.searchForm.page + "&limit=" + self.searchForm.limits, self.searchForm).then(function(response) {
      ...
      handleSave: function(formName) {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            this.$axios.post('/BaseFolder/Create', this.addForm).then((res) => {
      ...
      handleUpdate: function(formName) {
        let self = this;
        this.$axios.post('/BaseFolder/Edit', this.editForm, {
          timeout: 1000 * 60 * 2
        }).then((res) => {
          self.$message.success(res.data.msg);
          self.handleSearch();
      ...
      handleDel: function() {
        ...
        this.$confirm('确认删除该记录吗?', '提示', {
          ...
          self.$axios.post('/BaseFolder/SetDeleted', qs.stringify({
            'ids': ids
          }, {
            indices: false
      ...
      handleSingleDelete(index, row) {
        ...
        self.$confirm('确认删除该记录吗?', '提示', {
          ...
          axios.post('/BaseFolder/SetDeleted', qs.stringify(para))
            .then(function(response) {
              self.$message.success(response.data.msg);
              self.handleSearch();
            })
